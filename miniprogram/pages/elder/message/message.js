  ...
  async loadSystemMessages() {
    try {
      ...
      const res = await db.collection('messages')
        .where({
          toUserId: userId,
          type: _.in(['system', 'health_alert', 'sos'])
        })
        .orderBy('createTime', 'desc')
        .limit(10)
        .get()
      ...
      const healthAlerts = all.filter(m => m.type === 'health_alert')
      const systemMessages = all.filter(m => m.type !== 'health_alert')
      this.setData({ systemMessages, healthAlerts })
    } catch (e) {
